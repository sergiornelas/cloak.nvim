	...
	for _, pattern in ipairs(M.opts.patterns) do
		if type(pattern.cloak_pattern) == "string" then
			...
			for i, inner_pattern in ipairs(pattern.cloak_pattern) do
				pattern.cloak_pattern[i] = type(inner_pattern) == "string"
						and { inner_pattern, replace = pattern.cloak_pattern.replace or pattern.replace }
					or inner_pattern
			end
		end
		vim.api.nvim_create_autocmd({ "BufRead", "TextChanged", "TextChangedI", "TextChangedP" }, {
...
M.cloak = function(pattern)
	...
	local function determine_replacement(length, prefix)
		local cloak_str = prefix
			.. M.opts.cloak_character:rep(tonumber(M.opts.cloak_length) or length - vim.fn.strchars(prefix))
	...
	for i, line in ipairs(lines) do
		...
		while
			...
			and i ~= M.opts.uncloaked_line_num
		do
			...
			for _, inner_pattern in ipairs(pattern.cloak_pattern) do
				local current_first, current_last, capture_group = line:find(inner_pattern[1], searchStartIndex)
				if
					current_first ~= nil
				...
				then
					...
					if M.opts.try_all_patterns == false then
						break
					end
				end
			end
